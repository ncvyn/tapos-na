...
describe("Timezone & DST seam", () => {
  describe("getTodayWeekday", () => {
    ...
    it("resolves correctly across DST fall-back transition", () => {
      ...
      expect(getTodayWeekday("America/New_York", beforeFallBack)).toBe(
        "sunday",
      );
  ...
  describe("getZonedClockTime", () => {
    it("formats the wall-clock time in a timezone as 24-hour HH:MM:SS", () => {
      ...
      expect(getZonedClockTime("America/Los_Angeles", instant)).toBe(
        "05:34:56",
      );
    });
    ...
    it("is DST-aware across the fall-back and spring-forward transitions", () => {
      ...
      expect(
        getZonedClockTime("America/New_York", new Date("2026-11-01T06:30:00Z")),
      ).toBe("01:30:00");
      // Spring-forward: 2026-03-08T14:00:00Z is 10:00 EDT (after the 2:00 EST → 3:00 EDT jump).
      expect(
        getZonedClockTime("America/New_York", new Date("2026-03-08T14:00:00Z")),
      ).toBe("10:00:00");
    });

    it("falls back to UTC formatting for an invalid timezone", () => {
      expect(
        getZonedClockTime("Invalid/Zone", new Date("2026-08-16T01:02:03Z")),
      ).toBe("01:02:03");
    });
  });
